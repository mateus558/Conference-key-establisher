...
void compute_gamma(){
  ...
  gamma_computed = true;
  MQTT.publish(TOPICO_ALPHA, _alpha.toString());
  MQTT.publish(TOPICO_Y, _y.toString());
  MQTT.publish(TOPICO_BETA, _beta.toString());
...
void mqtt_callback(char* topic, byte* payload, unsigned int length) 
{
    ...
    if(_topic.equals(TOPICO_NUSERS)){
      ...
      if(!gamma_computed && alpha.length() > 0 && beta.length() > 0 && delta.length() > 0 && totient_delta.length() > 0){
...
void loop() 
{   
    if(!session_key_computed && y.length() > 0 && gamma1.length() > 0 && delta.length() > 0 && alpha.length() > 0 && totient_delta.length() > 0){
        Serial.println("- Gamma and delta parameters received.");
        compute_gamma1();
    }
    MQTT.publish(TOPICO_Y,(char*) y.c_str());
